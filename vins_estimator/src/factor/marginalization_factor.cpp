...
void ResidualBlockInfo::Evaluate()
{
    ...
    // cost_function:ceres::CostFunction *cost_function;
    ...
    for (auto it : factors)
    {
        ...
        for (int i = 0; i < static_cast<int>(block_sizes.size()); i++)
        {
            ...
            if (parameter_block_data.find(addr) == parameter_block_data.end())
            {
                ...
                // memcpy：内存拷贝函数，data：目的地;it->parameter_blocks[i]:数据来源；
...
void* ThreadsConstructA(void* threadsstruct)
{
    ...
    {
        // 遍历每一个参数块，参数块可能是 机体位置、速度、旋转、零偏；相机特征点；
        // 像下面这样的形式
...
bool MarginalizationFactor::Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
{
    ...
    for (int i = 0; i < static_cast<int>(marginalization_info->keep_block_size.size()); i++)
    {
        ...
        // 如果x为6，则其中不包含四元数，如果包含四元数，则需要使用四元数乘法运算
        if (size != 7)
    ...
    // Technical Report:VINS-Mono 公式（24）中， r_p - H_p x
